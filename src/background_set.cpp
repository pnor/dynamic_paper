...
// Helper function to cast then insert into ParsingInfo
template <typename T>
...
static void updateParsingInfoWithYamlNode(const YAML::const_iterator yaml,
  ...
  if (key == DATA_DIRECTORY && yaml->second.IsScalar()) {
    insertIntoParsingInfo(yaml, parsingInfo.dataDirectory);
  } else if (key == IMAGES && yaml->second.IsSequence()) {
    insertIntoParsingInfo(yaml, parsingInfo.images);
  } else if (key == IMAGE) {
    insertIntoParsingInfo(yaml, parsingInfo.image);
  } else if (key == MODE) {
    insertIntoParsingInfo(yaml, parsingInfo.mode);
  } else if (key == ORDER) {
    insertIntoParsingInfo(yaml, parsingInfo.order);
  } else if (key == TIMES) {
    insertIntoParsingInfo(yaml, parsingInfo.timeStrings);
  } else if (key == TRANSITION) {
    insertIntoParsingInfo(yaml, parsingInfo.transition);
  } else if (key == TRANSITION_LENGTH) {
    insertIntoParsingInfo(yaml, parsingInfo.transitionLength);
  } else if (key == TYPE) {
    insertIntoParsingInfo(yaml, parsingInfo.type);
  }
}
